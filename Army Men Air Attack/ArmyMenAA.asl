...
init
{
	...
	{
		emu.Make<int>("Start", 0x000eb1ec);
		emu.Make<int>("MissionNumber", 0x000d0990);
		emu.Make<int>("MissionComplete", 0x000d7eb4);
...
start
{
	return old.Start == 0 && current.Start == 1;
...
split
{
    return old.MissionComplete == 0 && current.MissionComplete == 1;
}
